...
// GET /api/plots/geojson?farm_id=X
// FeatureCollection para Leaflet
...
// PUT /api/plots/:id
// Atualiza um talhão.
// A geometria só é alterada se vier no corpo do pedido;
// caso contrário, mantém-se o valor existente.
router.put(
  '/:id',
  param('id').isInt(),
  asyncHandler(async (req, res) => {
    ...
    if (!result.rows.length) {
      return res.status(404).json({
        error:
          'Talhão não encontrado ou sem acesso à exploração.'
      });
    }
